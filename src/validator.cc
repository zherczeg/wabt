...
Result ComponentValidator::CheckComponent() {
  const ComponentDefList* def_list = current_component_;
  std::vector<size_t> parent_data;
  size_t i = 0;

  while (true) {
    while (i < def_list->Size()) {
      const ComponentDef* definition = def_list->Get(i++);
      ComponentSection section = definition->section();

      switch (section) {
        case ComponentSection::CoreModule: {
          const Module* module = definition->AsCoreModule()->module();
          CHECK_RESULT(ValidateModule(module, errors_, options_));
          validator_.OnCoreModule();
          break;
        }
        ...
        case ComponentSection::Instance: {
          if (section == ComponentSection::Instance &&
            ...
            uint32_t argument_count =
                static_cast<uint32_t>(instance->Arguments().size());
            CHECK_RESULT(validator_.OnInlineInstance(argument_count));

            for (auto& arg : instance->Arguments()) {
              std::string_view* version_suffix = nullptr;
              std::string_view version_suffix_data;
              if (arg.version_suffix != nullptr) {
                version_suffix_data = *arg.version_suffix;
                version_suffix = &version_suffix_data;
              }

              CHECK_RESULT(validator_.OnInlineInstanceArg(
                  arg.name.ToStringLoc(), version_suffix, arg.sort, arg.index));
            }
            break;
          }
          ...
          if (definition->instance() == ComponentDef::Instance::Inline) {
            CHECK_RESULT(validator_.OnInlineCoreInstance(argument_count));
        ...
        case ComponentSection::Component:
          ...
          def_list = definition->AsComponent();
          i = 0;
    ...
    if (def_list->GetParent() == nullptr) {
      assert(parent_data.empty());
      break;
    }

    if (def_list->section() == ComponentSection::Component) {
      validator_.EndComponent();
    } else if (def_list->type() == ComponentTypeDef::Instance) {
      validator_.EndInstanceType();
    } else {
      assert(def_list->type() == ComponentTypeDef::Component);
...
Result ComponentValidator::CheckAlias(const ComponentDef* definition) {
  ...
  if (definition->alias() == ComponentDef::Alias::Export) {
    return validator_.OnAliasExport(alias->GetLocation(), alias->sort(),
                                    alias->InstanceIndexLoc(),
                                    alias->ExportNameLoc().ToStringLoc());
  }

  return validator_.OnAliasCoreExport(alias->GetLocation(), alias->sort(),
                                      alias->InstanceIndexLoc(),
                                      alias->ExportNameLoc().ToStringLoc());
}
  ...
  while (true) {
    ...
    switch (type->type()) {
      ...
      case ComponentTypeDef::Resource: {
        ...
        CHECK_RESULT(validator_.OnResourceType(
            resource->GetLocation(), resource->Rep(), resource->DtorLoc()));
        break;
      }
      case ComponentTypeDef::ResourceAsync: {
        const ComponentTypeResource* resource = type->AsTypeResource();
        CHECK_RESULT(validator_.OnResourceAsyncType(
            resource->GetLocation(), resource->Rep(), resource->DtorLoc(),
      ...
      if (i < def_list->Size()) {
        switch (def_list->Get(i)->section()) {
          case ComponentSection::Type:
            ...
            break;
          case ComponentSection::Alias:
            CHECK_RESULT(CheckAlias(def_list->Get(i)));
            type_stack.back() = i + 1;
            continue;
          case ComponentSection::Import:
          case ComponentSection::Export:
            CHECK_RESULT(CheckExternal(def_list->Get(i)->AsExternal()));
            type_stack.back() = i + 1;
            continue;
          default:
            assert(0);
...
Result ComponentValidator::CheckExternal(const ComponentExternal* external) {
  ...
  if (external->section() == ComponentSection::Export) {
    ...
    if (external->ExportIndexLoc().index != kInvalidIndex) {
      export_info_data.sort = external->ExportSort();
